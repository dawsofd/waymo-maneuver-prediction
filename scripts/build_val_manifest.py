...
from the Waymo E2E validation set.
...
Output: data/val_manifest.json

Usage:
    python scripts/build_val_manifest.py
    python scripts/build_val_manifest.py --sample 200  # for testing
"""
...
OUTPUT_PATH = 'data/val_manifest.json'
...
def _load_future(fname, val_dir):
...
def build_manifest(val_dir, seq_ids, sequences):
    manifest = {}
    dropped = 0
    ...
    for i, seq_id in enumerate(seq_ids):
        ...
        for frame_idx, fname in seq_frames:
            fut = _load_future(fname, val_dir)
            if fut is None:
                continue
            xs, ys, ts = fut
...
def main():
    ...
    print(f'Scanning {args.val_dir}...')
    files = sorted([f for f in os.listdir(args.val_dir) if f.endswith('.npz')])
    ...
    for fname in files:
        ...
        frame_str = fname.rsplit('_', 1)[1].replace('.npz', '')

        # Skip duplicate files such as frame 91(1).npz that may be created
        # when the processed dataset is copied or extracted more than once.
    ...
    if skipped_files:
        print(f'Skipped {skipped_files:,} duplicate files with invalid frame names')
    ...
    manifest, dropped = build_manifest(args.val_dir, seq_ids, sequences)
