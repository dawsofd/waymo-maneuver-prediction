"""
Extract one sample frame per scenario class from a WOD-E2E tfrecord shard.
Uses naive JPEG byte-scanning (camera_0 / first JPEG in each record).

Usage:
    python scripts/extract_samples.py
    python scripts/extract_samples.py --shard val_202504211843.tfrecord-00000-of-00093
"""
import argparse
import json
import os
from collections import defaultdict
from pathlib import Path

import tensorflow as tf

DATA_DIR = Path(__file__).parent.parent / "data"
DEFAULT_SHARD = "val_202504211843.tfrecord-00000-of-00093"


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--shard", default=DEFAULT_SHARD)
    parser.add_argument("--output_dir", default=str(DATA_DIR / "sample_frames"))
    parser.add_argument("--target_per_class", type=int, default=1)
    return parser.parse_args()


def main():
    args = parse_args()
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    with open(DATA_DIR / "val_sequence_name_to_scenario_cluster.json") as f:
        labels = json.load(f)

    saved = {}
    dataset = tf.data.TFRecordDataset(str(DATA_DIR / args.shard))

    for raw in dataset:
        b = raw.numpy()
        example = tf.train.Example()
        example.ParseFromString(b)
        keys = list(example.features.feature.keys())
        if not keys:
            continue
        seq_id = keys[0].split("-")[0]
        if seq_id not in labels:
            continue
        cluster = labels[seq_id]["scenario_cluster"]
        if cluster in saved:
            continue

        idx = b.find(b"\xff\xd8\xff")
        if idx == -1:
            continue
        end = b.find(b"\xff\xd9", idx) + 2

        safe_name = cluster.replace(" ", "_").replace("/", "_")
        fname = output_dir / f"{safe_name}.jpg"
        with open(fname, "wb") as f:
            f.write(b[idx:end])
        print(f"Saved: {fname} (seq: {seq_id})")
        saved[cluster] = str(fname)

        if len(saved) >= 10:
            break

    print(f"\nDone: {list(saved.keys())}")


if __name__ == "__main__":
    main()
